...
class FeatureYokogawa:
    ...
    def yokogawa_start(self, mode="230V",
                        dir="/home/lab/markus/power", file=None):
        """
...
'''
class FeatureIntelPCM:
    def __init__(self):
        self._intelpcm, self._intelpcm_fname = None, None
        super().__init__()

    def intelpcm_start(self, user="markus", prefix="pyAPI",
                    datetime=None):
        if self._intelpcm is None:              # didn't run yet
            self.call("modprobe msr; echo 0 > /proc/sys/kernel/nmi_watchdog", sudo=True)
        elif self._intelpcm:                    # still running
            self.intelpcm_stop()
        self._intelpcm_fname = os.path.join("/home/lab", user, "intelpcm",
                "{}_{}.csv".format(prefix,
                time.strftime("%Y-%m-%d_%H-%M-%S") if datetime is None else datetime))
        self.start("/home/lab/intelpcm/pcm.x -r -csv={}".format(self._intelpcm_fname), sudo=True, delay=1.0)
        self._intelpcm = True

    def intelpcm_stop(self):
        # pcm is no child process of sudo anymore...
        self.call("pkill pcm.x; chown lab:lab {}".format(self._intelpcm_fname), sudo=True)
        self._intelpcm = False

    def intelpcm_save(self, dst):
        """Download the remote file to given local destination."""
        if self._intelpcm_fname is None:
            raise Exception("There is no file to download.")
        if os.path.isdir(dst):
            dst = os.path.join(dst, os.path.split(self._intelpcm_fname)[1])
        return subprocess.call(["scp", "{}:{}".format(self.host, self._intelpcm_fname), dst])
...
class FeatureWT230:
    ...
    def WT230_start(self, mode="230V", user="markus", prefix="pyAPI",
                    datetime=None):
        """
...
class Odroid(SshDevice, FeatureDstat):
    """
    SshDevice for the Odroids with integrated Dstat.

    :param ip: ip address of the Odroid
    :type ip: str
    """
    def __init__(self, ip):
        super().__init__("odroid@{}".format(ip), password="odroid")
